(function () {
  ...
  const css = `
    ...
    * { box-sizing: border-box; }
    ...
    .ai-toc a:hover { color: var(--ai-accent); background: var(--ai-panel-soft); }
    .ai-toc a.active {
      color: var(--ai-accent);
      font-weight: 600;
    ...
    .ai-document :is(h1, h2, h3) { position: relative; }
    .ai-anchor {
    ...
    .ai-anchor:hover { color: var(--ai-accent); }
    .ai-document h1:hover .ai-anchor,
    .ai-document h2:hover .ai-anchor,
    .ai-document h3:hover .ai-anchor { opacity: 0.7; }
    .ai-anchor.copied { opacity: 1; color: var(--ai-green); }
    ...
    body.ai-no-numbers .ai-component-header::before { display: none; }
    body.ai-no-numbers .ai-component-header { padding-left: 14px; }
    ...
    .ai-exec-summary[data-expanded="false"] .ai-exec-body { display: none; }
    .ai-exec-head {
      display: flex;
      align-items: center;
      justify-content: space-between;
    ...
    .ai-exec-toggle:hover { color: var(--ai-ink); }
    .ai-exec-body { margin-top: 10px; }
    .ai-exec-takeaway { margin: 0 0 10px; font-size: 14px; line-height: 1.55; color: var(--ai-ink); }
    .ai-exec-takeaway strong { color: var(--ai-accent); margin-right: 4px; }
    .ai-exec-metrics {
      display: grid;
    ...
    .ai-exec-m-label { font-size: 10px; color: var(--ai-muted); text-transform: uppercase; letter-spacing: 0.08em; }
    .ai-exec-m-value { font-size: 17px; font-weight: 700; color: var(--ai-ink); font-variant-numeric: tabular-nums; }
    body.ai-no-summary .ai-exec-summary { display: none !important; }
    ...
    .ai-export-btn:hover { color: var(--ai-accent); border-color: var(--ai-accent); }
    .ai-export-btn.copied { color: var(--ai-green); border-color: var(--ai-green); }
    script[data-ai-csv] { display: none; }
    .ai-component-header > .ai-export-btn { margin-left: auto; }
    .ai-component-header > .ai-search ~ .ai-export-btn { margin-left: 0; }
    ...
    @media print {
      ...
      :root, :root[data-ai-theme="dark"] {
        ...
        --ai-pre-bg: #f4f4f7;
        --ai-pre-fg: #111111;
        --ai-codeblock-head-bg: #eaeaef;
        --ai-codeblock-head-fg: #333333;
        --ai-codeblock-head-border: #d6d6dc;
        color-scheme: light;
      }
      body.ai-output-body { background: #fff; color: #111; font-size: 11pt; }
      .ai-topbar, .ai-toc, .ai-actions, .ai-source, .ai-search,
      .ai-codeblock-copy, .ai-exec-toggle, [data-ai-export-csv] { display: none !important; }
      .ai-shell { min-height: auto; }
      .ai-layout { grid-template-columns: 1fr; padding: 0; }
      .ai-document { padding: 0; max-width: none; }
      .ai-component, .ai-table, .ai-callout, .ai-metric-card,
      .ai-trend-card, .ai-status-card, .ai-gauge-card {
        box-shadow: none !important;
        break-inside: avoid;
        page-break-inside: avoid;
      }
      .ai-metric-grid, .ai-trend-grid, .ai-status-grid, .ai-gauge-grid {
      ...
      .ai-report-header {
        ...
        break-after: page;
        margin: 0 0 0 0;
        min-height: calc(100vh - 80mm);
        display: flex;
        flex-direction: column;
        justify-content: space-between;
      }
      .ai-report-header .ai-rh-title { color: #111; font-size: 28pt; }
      .ai-report-header .ai-rh-subtitle { color: #444; font-size: 13pt; }
      .ai-report-header .ai-rh-chip {
        background: #ecedf5 !important;
        color: #333 !important;
        border-color: #d6d8e1 !important;
      }
      .ai-report-header .ai-rh-key { color: #555 !important; }
      .ai-report-header .ai-rh-status-final { background: #d6f0dc !important; color: #14532d !important; }
      .ai-report-header .ai-rh-status-review { background: #fef3c7 !important; color: #78350f !important; }
      .ai-report-header .ai-rh-class-confidential,
      .ai-report-header .ai-rh-class-restricted { background: #fee2e2 !important; color: #7f1d1d !important; }
      .ai-document > h1 { page-break-before: always; break-before: page; }
      .ai-document > h1:first-child { page-break-before: avoid; break-before: avoid; }
      .ai-report-header + h1 { page-break-before: avoid; break-before: avoid; }
      .ai-component-header::before { display: none; }
      .ai-callout, .ai-trend-card.ai-tone-good, .ai-trend-card.ai-tone-warn, .ai-trend-card.ai-tone-bad {
        border: 1px solid #ccc !important;
      }
      .ai-callout-success { border-left: 4px solid #16a34a !important; }
      .ai-callout-warning { border-left: 4px solid #f97316 !important; }
      .ai-callout-danger  { border-left: 4px solid #dc2626 !important; }
      .ai-codeblock { break-inside: avoid; }
      .ai-codeblock pre { background: #f4f4f7; color: #111; padding: 10px 12px; }
      a { color: inherit; text-decoration: underline; }
      a[href^="http"]::after { content: " (" attr(href) ")"; font-size: 9pt; color: #666; }
      .ai-exec-summary { display: none !important; }
    }
  `;
  ...
  function formatValue(value, format, locale) {
    ...
    try {
      ...
      if (fmt && fmt.startsWith("currency:")) {
  ...
  function csvSlugFilename(title, fallback) {
    const base = String(title || fallback || "report")
      .toLowerCase()
      .replace(/[^\w一-龥-]+/g, "-")
      .replace(/^-+|-+$/g, "")
      .slice(0, 60);
  ...
  function csvAttachment(csvText, filename) {
    const safe = String(csvText).replace(/<\/script/gi, "<\\/script");
    return `<script type="text/csv" data-ai-csv data-ai-csv-name="${escapeHtml(filename)}">${safe}</script>`;
  }
  function csvButton() {
    return `<button class="ai-export-btn" type="button" data-ai-export-csv aria-label="Export CSV" title="Export CSV">⤓ CSV</button>`;
  }
  ...
  function renderChart(data) {
    ...
    const csvRows = data.type === "pie" || data.type === "donut"
      ? [["label", "value"], ...data.slices.map((s) => [s.label, s.value])]
      : [["x", ...data.series.map((s) => s.name)], ...data.x.map((x, i) => [x, ...data.series.map((s) => s.data[i])])];
    const attachment = csvAttachment(csvSerialize(csvRows), csvSlugFilename(data.title, "chart"));
    return componentShell(title || L("defaultChartTitle"), subtitle, csvButton(),
      `<div class="ai-chart-wrap"><svg class="ai-chart-svg" viewBox="${out.viewBox}" role="img" aria-label="${escapeHtml(title || L("defaultChartTitle"))}">${out.svgBody}</svg></div>${out.legend}${caption}${attachment}`);
  }
  ...
  function renderTable(type, data) {
    ...
    const searchInput = data.filterable === false ? "" : `<input class="ai-search" type="search" placeholder="${escapeHtml(L("filter"))}" data-ai-filter="${id}">`;
    const controls = searchInput + csvButton();
    const title = data.title || L("defaultTableTitle");
    const subtitle = data.subtitle || L("defaultTableSubtitle");
    const csvRows = [data.columns.slice(), ...data.rows.map((r) => r.slice())];
    const attachment = csvAttachment(csvSerialize(csvRows), csvSlugFilename(data.title, "table"));
  ...
  function renderTrendCard(data) {
    ...
    const csvRows = [["label", "value", "format", "delta_value", "delta_label", "tone", "note"]];
    data.items.forEach((item) => csvRows.push([
      item.label, item.value, item.format || "",
      item.delta ? item.delta.value : "",
      item.delta && item.delta.label ? item.delta.label : "",
      item.tone || "", item.note || ""
    ]));
    const attachment = csvAttachment(csvSerialize(csvRows), csvSlugFilename(data.title, "trend"));
    const head = `
        <div class="ai-component-header">
    ...
    return `<section class="ai-component ai-trend-grid-wrap" aria-label="${escapeHtml(data.title || "Trend metrics")}">${head}<div class="ai-trend-grid">${cards}</div>${attachment}</section>`;
  }
  ...
  function renderStatusGrid(data) {
    ...
    const csvRows = [["label", "status", "value", "note"], ...data.items.map((it) => [it.label, it.status, it.value || "", it.note || ""])];
    const attachment = csvAttachment(csvSerialize(csvRows), csvSlugFilename(data.title, "status"));
    const head = `
        <div class="ai-component-header">
          <div><h3 class="ai-component-title">${escapeHtml(data.title || "")}</h3></div>
          ${csvButton()}
    ...
    return `<section class="ai-component ai-status-grid-wrap" aria-label="${escapeHtml(data.title || "Status grid")}">${head}<div class="ai-status-grid">${cards}</div>${attachment}</section>`;
  }
  ...
  function renderActionItems(data) {
    ...
    const csvRows = [["task", "owner", "due", "status", "priority"], ...data.items.map((it) => [it.task, it.owner || "", it.due || "", it.status || "todo", it.priority || ""])];
    const attachment = csvAttachment(csvSerialize(csvRows), csvSlugFilename(data.title, "action-items"));
    const head = `
        <div class="ai-component-header">
          <div><h3 class="ai-component-title">${escapeHtml(data.title || "")}</h3></div>
          ${csvButton()}
    ...
    return `<section class="ai-component ai-action-wrap">${head}<ul class="ai-action-list">${rows}</ul>${attachment}</section>`;
  }
  ...
  function renderComparison(data) {
    ...
    const csvHeader = ["criterion", ...data.options, "weight"];
    if (data.recommended) csvHeader.push("recommended");
    const csvRows = [csvHeader];
    data.criteria.forEach((c) => {
      const row = [c.label, ...c.values.map((v) => v), c.weight !== undefined ? c.weight : ""];
      if (data.recommended) row.push(data.recommended);
      csvRows.push(row);
    });
    const attachment = csvAttachment(csvSerialize(csvRows), csvSlugFilename(data.title, "comparison"));
    const head = `
      <div class="ai-component-header">
        <div>
    ...
    return `<section class="ai-component ai-comparison-wrap">${head}<div class="ai-table-wrap"><table class="ai-table ai-comparison"><thead><tr><th></th>${headers}</tr></thead><tbody>${rows}</tbody></table></div>${attachment}</section>`;
  }
  ...
  function renderGauge(data) {
    ...
    const csvRows = [["label", "value", "target", "min", "max", "format", "tone", "note"], ...data.items.map((it) => [
      it.label, it.value,
      it.target !== undefined ? it.target : "",
      it.min !== undefined ? it.min : "",
      it.max !== undefined ? it.max : "",
      it.format || "", it.tone || "", it.note || ""
    ])];
    const attachment = csvAttachment(csvSerialize(csvRows), csvSlugFilename(data.title, "gauge"));
    const head = `
        <div class="ai-component-header">
          <div><h3 class="ai-component-title">${escapeHtml(data.title || "")}</h3></div>
          ${csvButton()}
    ...
    return `<section class="ai-component ai-gauge-wrap">${head}<div class="ai-gauge-grid">${cards}</div>${attachment}</section>`;
  }
  ...
  function renderFunnel(data) {
      ...
      return `
        <div class="ai-funnel-row">
          ...
          <div class="ai-funnel-track">
            <div class="ai-funnel-bar-wrap"><div class="ai-funnel-bar" style="width:${width.toFixed(1)}%;background:${color};"></div></div>
            <span class="ai-funnel-value">${escapeHtml(valueText)}</span>
          </div>
          <div class="ai-funnel-conv">${stepLabel}${overallLabel}</div>
        </div>
      `;
    }).join("");
    const csvRows = [["stage", "value", "step_pct", "overall_pct", "tone", "note"]];
    data.stages.forEach((stage, i) => {
      const prev = i === 0 ? null : data.stages[i - 1].value;
      const step = prev && prev > 0 ? +(stage.value / prev).toFixed(4) : "";
      const overall = first > 0 ? +(stage.value / first).toFixed(4) : "";
      csvRows.push([stage.label, stage.value, step, overall, stage.tone || "", stage.note || ""]);
    });
    const attachment = csvAttachment(csvSerialize(csvRows), csvSlugFilename(data.title, "funnel"));
    const head = `
      <div class="ai-component-header">
        <div>
    ...
    return `<section class="ai-component ai-funnel-wrap">${head}<div class="ai-funnel">${rows}</div>${attachment}</section>`;
  }
  ...
  function renderWaterfall(data) {
    ...
    const csvRows = [["label", "value", "kind", "cumulative"]];
    let runningCum = 0;
    bars.forEach((bar) => {
      if (bar.kind === "start" || bar.kind === "subtotal" || bar.kind === "end") runningCum = bar.value;
      else runningCum += bar.value;
      csvRows.push([bar.label, bar.value, bar.kind, runningCum]);
    });
    const attachment = csvAttachment(csvSerialize(csvRows), csvSlugFilename(data.title, "waterfall"));
    const head = `
      <div class="ai-component-header">
        <div>
    ...
    return `<section class="ai-component ai-waterfall-wrap">${head}<div class="ai-chart-wrap"><svg class="ai-chart-svg" viewBox="0 0 ${W} ${H}" role="img" aria-label="${escapeHtml(data.title || "Waterfall")}">${connectors.join("")}${svgBars.join("")}</svg></div>${attachment}</section>`;
  }
  ...
  function renderHeatmap(data) {
    ...
    const csvRows = [["", ...data.xLabels]];
    data.rows.forEach((row, j) => csvRows.push([data.yLabels[j], ...row]));
    const attachment = csvAttachment(csvSerialize(csvRows), csvSlugFilename(data.title, "heatmap"));
    const head = `
      <div class="ai-component-header">
        <div>
    ...
    return `<section class="ai-component ai-heatmap-wrap">${head}<div class="ai-chart-wrap"><svg class="ai-chart-svg" viewBox="0 0 ${W} ${H}" role="img" preserveAspectRatio="xMidYMid meet">${xHeaders}${yHeaders}${cells}</svg></div>${legend}${attachment}</section>`;
  }
  ...
  function injectExecutiveSummary(target) {
    const doc = target.querySelector(".ai-document");
    if (!doc) return;
    const header = doc.querySelector(".ai-report-header");
    const firstCallout = doc.querySelector(".ai-callout");
    const firstMetric = doc.querySelector(".ai-trend-grid, .ai-metric-grid");
    if (!firstCallout && !firstMetric) return;
    const calloutTitleEl = firstCallout ? firstCallout.querySelector(".ai-component-title") : null;
    const calloutBodyEl = firstCallout ? firstCallout.querySelector(".ai-component-body p") : null;
    const calloutTitle = calloutTitleEl ? calloutTitleEl.textContent.trim() : "";
    const calloutBody = calloutBodyEl ? calloutBodyEl.textContent.trim() : "";
    const bodyExcerpt = calloutBody.length > 260 ? calloutBody.slice(0, 260).trim() + "…" : calloutBody;
    let metrics = [];
    if (firstMetric) {
      if (firstMetric.classList.contains("ai-trend-grid")) {
        metrics = Array.from(firstMetric.querySelectorAll(".ai-trend-card")).slice(0, 4).map((c) => ({
          label: (c.querySelector(".ai-trend-label") || {}).textContent || "",
          value: (c.querySelector(".ai-trend-value") || {}).textContent || ""
        }));
      } else {
        metrics = Array.from(firstMetric.querySelectorAll(".ai-metric-card")).slice(0, 4).map((c) => ({
          label: (c.querySelector(".ai-metric-label") || {}).textContent || "",
          value: (c.querySelector(".ai-metric-value") || {}).textContent || ""
        }));
      }
    }
    if (!bodyExcerpt && !metrics.length) return;
    const summaryTitle = currentLocale.startsWith("zh") ? "执行摘要" : "Executive summary";
    const metricsHtml = metrics.map((m) => `<div class="ai-exec-metric"><span class="ai-exec-m-label">${escapeHtml(m.label)}</span><span class="ai-exec-m-value">${escapeHtml(m.value)}</span></div>`).join("");
    const html = `
      <section class="ai-exec-summary" data-expanded="true">
        <header class="ai-exec-head">
          <h3>${escapeHtml(summaryTitle)}</h3>
          <button class="ai-exec-toggle" type="button" aria-expanded="true" aria-label="${escapeHtml(summaryTitle)}">▾</button>
        </header>
        <div class="ai-exec-body">
          ${bodyExcerpt ? `<p class="ai-exec-takeaway">${calloutTitle ? `<strong>${escapeHtml(calloutTitle)}</strong>` : ""} ${escapeHtml(bodyExcerpt)}</p>` : ""}
          ${metricsHtml ? `<div class="ai-exec-metrics">${metricsHtml}</div>` : ""}
        </div>
      </section>
    `;
    const insertAfter = header || doc.firstElementChild;
    if (!insertAfter) return;
    insertAfter.insertAdjacentHTML("afterend", html);
    const card = doc.querySelector(".ai-exec-summary");
    const toggle = card.querySelector(".ai-exec-toggle");
  ...
  function bindTocHighlight(root) {
    const toc = root.querySelector(".ai-toc");
    if (!toc) return;
    const links = Array.from(toc.querySelectorAll("a[href^='#']"));
    if (!links.length || typeof IntersectionObserver === "undefined") return;
    const idMap = new Map();
    links.forEach((l) => idMap.set(l.getAttribute("href").slice(1), l));
    const targets = [];
    idMap.forEach((_, id) => {
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          const link = idMap.get(entry.target.id);
  ...
  function bindHeadingAnchors(root) {
    root.querySelectorAll(".ai-document :is(h1, h2, h3)[id]").forEach((h) => {
      if (h.querySelector(".ai-anchor")) return;
      const a = document.createElement("a");
      a.className = "ai-anchor";
      a.href = "#" + h.id;
      a.textContent = "#";
      ...
      a.addEventListener("click", async (event) => {
        ...
        if (navigator.clipboard) {
          try { await navigator.clipboard.writeText(url.toString()); } catch (_) { /* ignore */ }
        }
  ...
  function bindInteractions(root) {
    ...
    root.querySelectorAll("[data-ai-export-csv]").forEach((btn) => {
      btn.addEventListener("click", () => {
        const section = btn.closest(".ai-component, .ai-report-header");
        if (!section) return;
        const script = section.querySelector("script[data-ai-csv]");
        if (!script) return;
        const csv = script.textContent;
        const filename = script.getAttribute("data-ai-csv-name") || "report.csv";
  ...
  function render(markdown, options) {
    ...
    if (options.numbering === false) document.body.classList.add("ai-no-numbers");
    if (options.summary === false) document.body.classList.add("ai-no-summary");
    ...
    if (options.summary !== false) injectExecutiveSummary(target);
    bindTocHighlight(target);
    bindHeadingAnchors(target);
